...
export default function UsersPage() {
  ...
  const handleApiError = useCallback(
    (error: unknown, fallbackMessage: string) => {
      if (error instanceof AxiosError) {
        ...
        const responseBody = error.response?.data as { message?: string; error?: string } | undefined
        const description = responseBody?.message ?? responseBody?.error ?? fallbackMessage

        if (status === 401) {
          toast({ title: 'Unauthorized', description: 'Please sign in again to continue.', variant: 'destructive' })
          router.push('/login')
          return
        }

        if (status === 403) {
          toast({ title: 'Access denied', description: 'You do not have permission to view this resource.', variant: 'destructive' })
          router.push('/')
          return
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsedUser.role !== USER_ROLES.SUPER_ADMIN && parsedUser.role !== USER_ROLES.ADMIN) {
        toast({ title: 'Unauthorized', description: "You don't have permission to access the users area.", variant: 'destructive' })
        router.push('/')
        setIsCheckingAuth(false)
        return
      }
    ...
    try {
      ...
      if (updatedUser) {
        ...
        setMeta((prev) => {
          ...
          const wasAdmin = previousUser.role === USER_ROLES.ADMIN || previousUser.role === USER_ROLES.SUPER_ADMIN
          const isAdmin = updatedUser.role === USER_ROLES.ADMIN || updatedUser.role === USER_ROLES.SUPER_ADMIN
      ...
      toast({ title: 'User updated', description: `User status changed to ${status}.`, variant: 'success' })
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    const proceed = window.confirm('Are you sure you want to delete this user? This action cannot be undone.')
    if (!proceed) {
      return
    }
    ...
    try {
      ...
      setMeta((prev) => {
        ...
        if (deletedUser) {
          ...
          if (deletedUser.role === USER_ROLES.ADMIN || deletedUser.role === USER_ROLES.SUPER_ADMIN) {
            nextCounts.admins = Math.max(0, nextCounts.admins - 1)
          }
        }
      ...
      toast({ title: 'User deleted', description: 'The user account has been removed.', variant: 'success' })
    } catch (error) {
  ...
  const admins = users.filter((user: UserRecord) => user.role === USER_ROLES.ADMIN || user.role === USER_ROLES.SUPER_ADMIN).length
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <main className="container mx-auto px-4 py-8">
        <UsersHeader onRefresh={handleRefresh} isRefreshing={isRefreshing} />
        <UsersStats total={stats.total} active={stats.active} admins={stats.admins} loading={isLoading && users.length === 0} />
        <UsersTable
          users={users}
          meta={meta ?? undefined}
