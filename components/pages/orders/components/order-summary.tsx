...
export const OrderSummary = memo(
  ({ control, setValue, buyers, selectedProfiles, nextOrderNumber, orderProfiles }: OrderSummaryProps) => {
    ...
    const calculations = useMemo(() => {
      ...
      const burningWastagePercent =
        totalWeightSum > 0 ? (burningWeightTotal / totalWeightSum) * 100 : 0;

      return { profilesTotal, total, grandTotal, burningWeightTotal, burningWastagePercent };
    }, [selectedProfiles, quantity, profit, orderProfiles]);
            ...
            {profit > 0 && (
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-green-600 dark:text-green-400">
                  ₹{(calculations.grandTotal - calculations.total).toFixed(2)}
                </span>
              </div>
            )}
            ...
            <div className="pt-3 border-t">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">
                  {calculations.burningWeightTotal.toFixed(2)} kg
                </span>
              </div>
              <div className="flex justify-between items-center mt-2">
                <span className="text-sm text-muted-foreground">Burning Wastage</span>
                <span className="font-semibold">
                  {calculations.burningWastagePercent.toFixed(2)}%
                </span>
              </div>
            </div>
          </div>
